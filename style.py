...
def init_style():
    ...
    style.map(
        "Secondary.TButton",
        background=[('active', PALETTE["background"]), ('pressed', PALETTE["background"])], # Background changes to lighter shade
    ...
    style.map(
        "Danger.TButton",
        background=[('active', "#c82333"), ('pressed', "#bd2130")], # Darker red on hover
        foreground=[('active', PALETTE["text_light"])],
    ...
    style.map(
        ...
        background=[('hover', PALETTE["primary_dark"])] # Background of the dropdown button
    )
    ...
    style.map(
        ...
        background=[('active', PALETTE["background"])], # Active background remains same
        indicatorcolor=[('selected', PALETTE["primary"]), ('disabled', PALETTE["accent"])]
    )
    ...
    style.map(
        "TNotebook.Tab",
        background=[('selected', PALETTE["primary"]), ('active', PALETTE["border"])], # Selected tab is primary, hover is border
        foreground=[('selected', PALETTE["text_light"]), ('active', PALETTE["text"])],
        expand=[('selected', [0, 0, 0, 0])] # No expansion on selected tab
    )
    style.configure("TNotebook.Client", background=PALETTE["surface"], borderwidth=0) # Background of the tab content area
...
#     style.map(
#         "Menu.TButton",
#         background=[("active", "darkorange"), ("pressed", "red")],
